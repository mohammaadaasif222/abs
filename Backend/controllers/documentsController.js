  ...
  try {
    ...
    db.query(query, [class_name, time, fees, id], (err, results) => {
      ...
      if (results.affectedRows === 0) {
        return res.status(404).json({ message: "No fee structure found with the given id" });
      }

      res.status(200).json({ message: "Fee structure updated successfully", result: results });
    });
  } catch (error) {
    console.log(error);
    res.status(500).json({ error: error.message });
